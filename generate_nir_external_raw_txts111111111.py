...
def _make_botsort_no_reid_yaml() -> str:
    ...
    cfg["with_reid"]   = False
    cfg["gmc_method"]  = "none"   # 关闭光流 GMC，显著降低内存占用
    ...
    _yaml.dump(cfg, tmp)
    tmp.close()
    print(f"[botsort] 临时 yaml（with_reid=False, gmc=none）: {tmp.name}")
    return tmp.name
...
def gen_botsort(model, image_paths, output_txt, conf, iou, imgsz) -> bool:
    print(f"\n[generate] ▶ BOTSORT-NIR  (botsort.yaml, with_reid=False, gmc=none)")
    print(f"           输出 → {output_txt}")

    # 释放 ByteTrack 遗留内存，避免 OOM
    import gc
    gc.collect()
    try:
        import torch
    ...
    # 使用目录路径作为 source（触发流式加载），避免 LoadPilAndNumpy 全量载入
    nir_dir = os.path.dirname(os.path.abspath(image_paths[0])) if image_paths else ""

    tmp_yaml = None
    try:
        tmp_yaml = _make_botsort_no_reid_yaml()
        results = model.track(
            source=nir_dir, tracker=tmp_yaml,
            conf=conf, iou=iou, imgsz=imgsz,
            stream=True, verbose=False, persist=False,
        )
        records = _ul_results_to_records(results)
    ...
    if n == 0:
        ...
        return False
    print(f"[generate] ✅ BOTSORT-NIR → {n} 行写入 {output_txt}")
    return True
...
def run_generate(nir_seq_dir, weights_path, out_dir,
                 trackers, conf, iou, imgsz) -> dict[str, bool]:
    try:
